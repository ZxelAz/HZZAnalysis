...
def compute_yield_table(
    ...
    print(np.sum(event_weights))  # Debug: Check sum of event weights
    df = pd.DataFrame({
    ...
    # Keep only columns/rows that exist in the yield table
...
def yield_table_run2_categorisation(
    ...
    """
    def _sum_event_weight_from_category_dict(category_dict: dict) -> float:
        total = 0.0
        for _, cat_arr in category_dict.items():
            if 'event_weight' in cat_arr:
                total += float(np.sum(cat_arr['event_weight']))
        return total
    ...
    total_input_yield = float(np.sum(arr['event_weight']))
    print(f"[Run2 yield debug] Input total event weight: {total_input_yield:.10f}")
    ...
    total_after_first_step = _sum_event_weight_from_category_dict(first_step_arr_dict)
    print(f"[Run2 yield debug] After first-step categorisation total event weight: {total_after_first_step:.10f}")
    print(f"[Run2 yield debug] First-step delta vs input: {total_after_first_step - total_input_yield:.10f}")

    second_step_arr_dict = second_step_categorisation(first_step_arr_dict)
    total_after_second_step = _sum_event_weight_from_category_dict(second_step_arr_dict)
    print(f"[Run2 yield debug] After second-step categorisation total event weight: {total_after_second_step:.10f}")
    print(f"[Run2 yield debug] Second-step delta vs first-step: {total_after_second_step - total_after_first_step:.10f}")
    print(f"[Run2 yield debug] Second-step delta vs input: {total_after_second_step - total_input_yield:.10f}")
    ...
    total_df_stage1p2 = float(df_stage1p2['event_weight'].sum()) if not df_stage1p2.empty else 0.0
    print(f"[Run2 yield debug] Reconstructed stage1.2 DataFrame total event weight: {total_df_stage1p2:.10f}")
    print(f"[Run2 yield debug] DataFrame delta vs second-step: {total_df_stage1p2 - total_after_second_step:.10f}")
    ...
    total_table_stage1p2_by_mode = float(yield_table_stage1p2_by_mode.to_numpy().sum())
    print(f"[Run2 yield debug] Stage1.2 by-mode table total event weight: {total_table_stage1p2_by_mode:.10f}")
    print(f"[Run2 yield debug] By-mode table delta vs DataFrame: {total_table_stage1p2_by_mode - total_df_stage1p2:.10f}")
    ...
    total_table_stage1p2_by_true_label = float(yield_table_stage1p2_by_true_label.to_numpy().sum())
    print(f"[Run2 yield debug] Stage1.2 by-true-label table total event weight (before combine/rename): {total_table_stage1p2_by_true_label:.10f}")
    print(f"[Run2 yield debug] By-true-label table delta vs DataFrame: {total_table_stage1p2_by_true_label - total_df_stage1p2:.10f}")
    ...
    # Final totals excluding summary row/column
    final_total_stage1p2_by_mode = float(yield_table_stage1p2_by_mode.drop(index=['Success (%)'], errors='ignore').to_numpy().sum())
    final_total_stage1p2_by_true_label = float(
        yield_table_stage1p2_by_true_label
        .drop(index=['Success (%)'], errors='ignore')
        .drop(columns=['Purity (%)'], errors='ignore')
        .to_numpy()
        .sum()
    )
    print(f"[Run2 yield debug] Final Stage1.2 by-mode total event weight (excluding Success row): {final_total_stage1p2_by_mode:.10f}")
    print(f"[Run2 yield debug] Final Stage1.2 by-true-label total event weight (excluding Success row/Purity column): {final_total_stage1p2_by_true_label:.10f}")
    print(f"[Run2 yield debug] Final by-mode delta vs input: {final_total_stage1p2_by_mode - total_input_yield:.10f}")
    print(f"[Run2 yield debug] Final by-true-label delta vs input: {final_total_stage1p2_by_true_label - total_input_yield:.10f}")

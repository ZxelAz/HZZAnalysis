...
def plot_stacked_histograms(
    ...
    # Get bin edges from the histogram axis
    # Note: There seems to be a bug in hist library where .edges returns wrong values
    # So we reconstruct them from the internal boost-histogram axis
    hist_axis = stacked_hists[0].axes[0]
    if hasattr(hist_axis, '_ax') and hasattr(hist_axis._ax, 'value'):
        # Access the internal boost-histogram axis which has correct values
        internal_ax = hist_axis._ax
        n_bins = internal_ax.size
        # Use the value() method to get bin edges
        bin_edges = np.array([internal_ax.value(i) for i in range(n_bins + 1)])
    else:
    ...
    # Gather signal histograms if provided and add to stacks.
    all_values_list = mc_values_list.copy()
    all_labels = process_labels.copy()
    all_errors_list = mc_errors_list.copy()
    
    if signal_hists:
        for sig_hist, label in zip(signal_hists, signal_labels):
            sig_values = sig_hist.values()
            sig_errors = np.sqrt(sig_hist.variances())
            all_values_list.append(sig_values)
            all_labels.append(label)
            all_errors_list.append(sig_errors)
    ...
    n_processes = len(all_values_list)
    colors = get_distinct_colors(n_processes)

    # Plot all histograms (backgrounds + signals) stacked together.
    hep.histplot(
        all_values_list,
        label=all_labels,
        bins=bin_edges,
        ...
        edgecolor="black",
        linewidth=1,
        yerr=all_errors_list,
        ax=ax_main,
        color=colors,
        alpha=1,
    )

    # Add an uncertainty band for the total (background + signal) if desired.
    total = np.sum(all_values_list, axis=0)
    total_var = np.sum([err**2 for err in all_errors_list], axis=0)
    ...
    # Add legend outside the plot area to avoid blocking the histogram
    handles, labels = ax_main.get_legend_handles_labels()
    ncols = max(2, min(4, len(labels) // 5 + 1))  # Adaptive columns based on number of labels
    ax_main.legend(
        loc="upper left", 
        bbox_to_anchor=(1.02, 1.0),  # Position to the right of plot
        fontsize=10, 
        ncols=1,
        labelspacing=0.3, 
        columnspacing=1.0,

...
def main():
    ...
    new_rdf = rdf.Filter("ZLLbestSSIdx > -1") \
        .Filter("ZLLCand_mass[ZLLbestSSIdx] > 105 && ZLLCand_mass[ZLLbestSSIdx] < 140") \
        .Define("ZZmass", "ZLLCand_mass[ZLLbestSSIdx]") \
        .Define("ZZVector", "ROOT::Math::PtEtaPhiMVector(ZLLCand_pt[ZLLbestSSIdx], ZLLCand_eta[ZLLbestSSIdx], ZLLCand_phi[ZLLbestSSIdx], ZLLCand_mass[ZLLbestSSIdx])") \
    ...
    # ZLL candidates do not have ME probability branches (ZZCand_P_* are always empty
    # in ZX CR files because ME probabilities are only computed for signal-region candidates)
    new_rdf = new_rdf \
        .Define("DVBF2j_ME", "-999.0f") \
        .Define("DVBF1j_ME", "-999.0f") \
        .Define("DWHh_ME", "-999.0f") \
        .Define("DZHh_ME", "-999.0f") \
        .Define("DVBF2j_ME_noC", "-999.0f") \
        .Define("DVBF1j_ME_noC", "-999.0f") \
        .Define("DWHh_ME_noC", "-999.0f") \
        .Define("DZHh_ME_noC", "-999.0f") \
        .Redefine("ZZCand_KD", "ZLLCand_KD[ZLLbestSSIdx]") \
        .Redefine("ZZCand_pt", "ZLLCand_pt[ZLLbestSSIdx]") \
        .Redefine("ZZCand_eta", "ZLLCand_eta[ZLLbestSSIdx]") \
        .Redefine("ZZCand_phi", "ZLLCand_phi[ZLLbestSSIdx]") \
        .Redefine("ZZCand_costheta1", "ZLLCand_costheta1[ZLLbestSSIdx]") \
        .Redefine("ZZCand_costheta2", "ZLLCand_costheta2[ZLLbestSSIdx]") \
        .Redefine("ZZCand_costhetastar", "ZLLCand_costhetastar[ZLLbestSSIdx]") \
        .Redefine("ZZCand_Phi1", "ZLLCand_Phi1[ZLLbestSSIdx]") \
        .Redefine("ZZCand_nExtraLep", "ZLLCand_nExtraLep[ZLLbestSSIdx]")
    
    
    # Define lepton features
    new_rdf = new_rdf \
                    .Define('Z1_lep_hi_idx', "Lepton_pt[ZLLCand_Z1l1Idx[ZLLbestSSIdx]] >= Lepton_pt[ZLLCand_Z1l2Idx[ZLLbestSSIdx]] ? ZLLCand_Z1l1Idx[ZLLbestSSIdx] : ZLLCand_Z1l2Idx[ZLLbestSSIdx]") \
                    .Define('Z1_lep_lo_idx', "Lepton_pt[ZLLCand_Z1l1Idx[ZLLbestSSIdx]] >= Lepton_pt[ZLLCand_Z1l2Idx[ZLLbestSSIdx]] ? ZLLCand_Z1l2Idx[ZLLbestSSIdx] : ZLLCand_Z1l1Idx[ZLLbestSSIdx]") \
                    .Define('Z2_lep_hi_idx', "Lepton_pt[ZLLCand_Z2l1Idx[ZLLbestSSIdx]] >= Lepton_pt[ZLLCand_Z2l2Idx[ZLLbestSSIdx]] ? ZLLCand_Z2l1Idx[ZLLbestSSIdx] : ZLLCand_Z2l2Idx[ZLLbestSSIdx]") \
                    .Define('Z2_lep_lo_idx', "Lepton_pt[ZLLCand_Z2l1Idx[ZLLbestSSIdx]] >= Lepton_pt[ZLLCand_Z2l2Idx[ZLLbestSSIdx]] ? ZLLCand_Z2l2Idx[ZLLbestSSIdx] : ZLLCand_Z2l1Idx[ZLLbestSSIdx]") \
                    ...
                    .Define("LepPt_4", "ZLLCand_extraLep1Idx[ZLLbestSSIdx] != -1 ? Lepton_pt[ZLLCand_extraLep1Idx[ZLLbestSSIdx]] : -999.0") \
                    .Define("LepEta_4", "ZLLCand_extraLep1Idx[ZLLbestSSIdx] != -1 ? Lepton_eta[ZLLCand_extraLep1Idx[ZLLbestSSIdx]] : -999.0") \
                    .Define("LepPhi_4", "ZLLCand_extraLep1Idx[ZLLbestSSIdx] != -1 ? Lepton_phi[ZLLCand_extraLep1Idx[ZLLbestSSIdx]] : -999.0") \
                    .Define("LepPdgId_4", "ZLLCand_extraLep1Idx[ZLLbestSSIdx] != -1 ? Lepton_pdgId[ZLLCand_extraLep1Idx[ZLLbestSSIdx]] : 0") \
                    .Define("LepPt_5", "ZLLCand_extraLep2Idx[ZLLbestSSIdx] != -1 ? Lepton_pt[ZLLCand_extraLep2Idx[ZLLbestSSIdx]] : -999.0") \
                    .Define("LepEta_5", "ZLLCand_extraLep2Idx[ZLLbestSSIdx] != -1 ? Lepton_eta[ZLLCand_extraLep2Idx[ZLLbestSSIdx]] : -999.0") \
                    .Define("LepPhi_5", "ZLLCand_extraLep2Idx[ZLLbestSSIdx] != -1 ? Lepton_phi[ZLLCand_extraLep2Idx[ZLLbestSSIdx]] : -999.0") \
                    .Define("LepPdgId_5", "ZLLCand_extraLep2Idx[ZLLbestSSIdx] != -1 ? Lepton_pdgId[ZLLCand_extraLep2Idx[ZLLbestSSIdx]] : 0") \
                    

    # Persist only the analysis features to keep the Snapshot JIT manageable
    columns_to_save = [
        # ZZ candidate features (scalars defined from ZLLCand at ZLLbestSSIdx)
    ...
    # Remove any existing output file to avoid XRootD zombie-state on EOS
    output_root_path = f"{output_path}.root"
    if os.path.exists(output_root_path):
        os.remove(output_root_path)
        print(f"Removed existing output file: {output_root_path}")

    # Snapshot is lazy - trigger execution
    snapshot_result = new_rdf.Snapshot("Events", output_root_path, columns_to_save)
    print("Snapshot created.")
    # Force execution and get event count
    n_events = snapshot_result.Count().GetValue()
    print(f"Saved {n_events} events to {output_root_path}")
